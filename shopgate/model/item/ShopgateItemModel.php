...
class ShopgateItemModel extends Shopgate_Model_Catalog_Product
{
     ...
     */
    function __construct($config, $languageId, $shopCurrency, $countryId, $zoneId, $exchangeRate)
    {
    ...
    /**
     * Takes all Gx-Customizer-Sets and returns all kinds of possible input fields indexed by the specific customizer set
     *
                ...
                foreach ($gxCustomizerGroup as $gxCustomizerSurfaceId => $gxCustomizerSurface) {
                    ...
                    foreach ($gxCustomizerSurfaceReordered as $gxCustomizerSurfaceElement) {
                        ...
                        if ($gxCustomizerSurfaceElement['elements_group_type'] == 'text') {
                            ...
                            if (!empty($gxCustomizerSurfaceElement['surfaces_name'])) {
                                $textSpacing = '';
                                if (!empty($label)) {
                                    $textSpacing = ' ';
                                }
                                // use uppercase for the surface name, since it is used as uppercase in the shop-frondend
                                $label =
                                    strtoupper($gxCustomizerSurfaceElement['surfaces_name']) . $textSpacing . $label;
                            }
                            ...
                            // now a label should be available for labeling the input field
                            $inputFields[$gxCustomizerGroupId]["input_field_{$inputFieldNumber}_type"]       = 'text';
                            $inputFields[$gxCustomizerGroupId]["input_field_{$inputFieldNumber}_number"]     =
                                'gxcust_el_val_id_#' .
                            ...
                            $inputFields[$gxCustomizerGroupId]["input_field_{$inputFieldNumber}_label"]      = $label;
                            $inputFields[$gxCustomizerGroupId]["input_field_{$inputFieldNumber}_infotext"]   =
                                $infotext;
                            $inputFields[$gxCustomizerGroupId]["input_field_{$inputFieldNumber}_required"]   = 0;
                            $inputFields[$gxCustomizerGroupId]["input_field_{$inputFieldNumber}_add_amount"] = 0;
                            
                            // clear label for the following input field
                            $label = '';
                        }
                    }
                }
        ...
        if (($this->config->getVariationType() == SHOPGATE_SETTING_VARIATION_TYPE_PROPERTY
            ...
            $qry
                ...
                SELECT
                    ...
                    `ppc`.`combi_price`,
                    `ppc`.`combi_image`,"
                . ($databaseHelper->checkColumn(TABLE_PRODUCTS_PROPERTIES_COMBIS, "combi_ean") ? "`ppc`.`combi_ean`,"
                    : "") .
                "`ppc`.`vpe_value` as `products_vpe_value`,
                    `vpe`.`products_vpe_name`,
                    `pv`.`value_price` AS `display_price`,
                    `pvd`.`values_name`," .
                (ShopgateTools::isGambioVersionLowerThan("2.1") ? "`pv`.`value_price_type` AS `display_price_type`,"
                    : "") .
                ...
                . ((!empty($optionsAsTextFieldIds)) ? " AND pa.options_id NOT IN (" . implode(',',
            ...
            $propertyList = array();
            while ($propertiyCombo = xtc_db_fetch_array($qResult)) {
                // Group by products combination id
                if (empty($propertyList[$propertiyCombo['products_properties_combis_id']])) {
                    $propertyList[$propertiyCombo['products_properties_combis_id']] = array();
                }
                $propertyList[$propertiyCombo['products_properties_combis_id']][] = $propertiyCombo;
            }
    ...
    public function getInputFields($item, $inputFieldsFromGxCustomizer, $asArray = false)
    {
        ...
        $qry = "
            ...
            . ((!empty($optionsAsTextfieldIds)) ? " OR pa.options_id IN (" . implode(',',
                    $optionsAsTextfieldIds
                ) . "))" : ")")
    ...
    {
        if (!empty($categoryData['products_sorting'])) {
            $orderBy1 = $categoryData['products_sorting'];
        } else {
            $orderBy1 = 'p.products_price';
        }
        if ($categoryData['products_sorting2'] == 'ASC') {
            $orderBy2 = 'DESC';
        } else {
            $orderBy2 = 'ASC';
        }
        
        $gm_navigator_sort = ' ORDER BY ' . $orderBy1 . ' ' . $orderBy2;
        ...
        while ($products_data = xtc_db_fetch_array($products_query, true)) {
            if ($products_data['products_id'] == $itemNumber) {
                return $i;
            }
            $i++;
    ...
    public function getProductDescription($item, $desc, $shortDesc, $exportProductsManagedFiles, $exportDescriptionType)
    {
        ...
        if ($exportProductsManagedFiles) {
            $this->shopgateLogger->log(
                "loading products content file links (enabled by config) ...", ShopgateLogger::LOGTYPE_DEBUG
            );
            $description       = '';
            ...
            while ($row = xtc_db_fetch_array($queryResult)) {
	            ...
	            $iconImageUrl = rtrim(HTTP_SERVER . DIR_WS_CATALOG, '/') . "/admin/html/assets/images/legacy/icons/icon_{$fileExt}.gif";
                ...
                if ($i == 0) {
                    ...
                    if ($product['properties_show_price'] == 'true') { // "true" or "false"/empty as string
                        ...
                        $priceDisplayOffset = number_format(
                            $price, $this->shopCurrency["decimal_places"], $this->shopCurrency["decimal_point"],
    ...
    public function generateProductQuery($limit, $offset, $uids)
    {
        ...
        $qry .= (!empty($optionsAsTextfieldIds))
            ? " AND pa.options_id NOT IN (" . implode(',', $optionsAsTextfieldIds) . ")"
            : "";
    ...
    public function getGxCustomizerSets()
    {
        ...
        if (!$databaseHelper->tableExists('gm_gprint_surfaces_groups')) {
            return array();
        }
        ...
        while ($row = xtc_db_fetch_array($qryResult)) {
            $subCategoryBlacklistIds = array_merge(
                ...
                $this->getBlacklistedSubCategoriyIds(
                    $row['categories_id'], $customerGroupId,
    ...
    public function setSeoBoost(&$gmSEOBoost, &$gmSEOBoostProductsEnabled)
    {
        ...
        if (file_exists(DIR_FS_CATALOG . '.htaccess') && function_exists('gm_get_conf')) {
            $gmSEOBoostProductsEnabled = gm_get_conf('GM_SEO_BOOST_PRODUCTS') ? 1 : 0;
        }
    }
    ...
    {
        return (STOCK_ALLOW_CHECKOUT == 'true' || STOCK_CHECK != 'true') ? 0 : 1;
    }
    ...
    {
        return (empty($item['gm_min_order']) || ceil($item['gm_min_order']) <= 0) ? '' : ceil($item['gm_min_order']);
    }
     ...
     * @param array $product
     *
     * @param bool  $buildDataForXml
     *
     * @return string|array
     */
    public function buildProperties($product, $buildDataForXml = false)
    {
        $properties     = array();
        $databaseHelper = new ShopgateDatabaseHelper();
        
        if (!$buildDataForXml) {
            // xml exports this as "AgeRating"
            if (!empty($product["products_fsk18"]) && $product["products_fsk18"] == 1) {
                $properties[] = preg_replace("/\:/", "", TEXT_FSK18) . "=>" . TEXT_YES;
            }
        }
        
        $filters = $this->config->getExportFiltersAsProperties();
        
        if (empty($filters)) {
            // for xml export, the age rating needs to be set in a separate fired method.
            // so we don't need to return this information here.
            return !$buildDataForXml ? implode("||", $properties) : array();
        }
        
        $filters = "('" . implode("', '", $filters) . "')";
        
        if ($databaseHelper->checkTables(
            array(
            ...
            # for older gambio versions
            $sql
                = "
                SELECT pfv.feature_value_id, fd.feature_id, fv.sort_order, fd.feature_name, fvd.feature_value_text, fi.categories_index
                FROM products_feature_value    AS pfv
                JOIN feature_index             AS fi  ON fi.products_id       = pfv.products_id
                ...
                AND fd.feature_name IN {$filters}
            ...
            if ($databaseHelper->checkTables(
                ...
                $sql
                    = "
                SELECT fd.feature_name, fvd.feature_value_text, fd.feature_id 
        ...
        $filters = array();
        if (empty($sql)) {
            // same as previous return statement
            return !$buildDataForXml ? implode("||", $properties) : array();
        }
        ...
        while ($row = xtc_db_fetch_array($result)) {
            if (!isset($filters[$row['feature_name']])) {
                $filters[$row['feature_name']] = array();
            }
            if (!in_array($row['feature_value_text'], $filters[$row['feature_name']])) {
                if (!$buildDataForXml) {
                    $filters[$row['feature_name']][] = $row['feature_value_text'];
                } else {
                    $filters[$row['feature_name']][] = $row;
                }
            }
        }
        ...
        foreach ($filters as $filterName => $filterValues) {
            if (!$buildDataForXml) {
                $properties[] = "$filterName=>" . implode(', ', $filterValues);
            } else {
                $properties[] = array("$filterName" => $filterValues);
            }
        }
        
        return !$buildDataForXml ? implode("||", $properties) : $properties;
    }
    ...
    public function getProductCodes($uid)
    {
        ...
        if ($databaseHelper->checkTable("products_item_codes")) {
    ...
    public function getRelatedShopItems($products_id, $asArray = false)
    {
        ...
        return !$asArray ? implode("||", $xSellIds) : $xSellIds;
    }
