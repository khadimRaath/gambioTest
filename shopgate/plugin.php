...
class ShopgatePluginGambioGX extends ShopgatePlugin
{
    ...
    public function startup()
    {
        ...
        $this->languageId = !empty($language['languages_id']) ? $language['languages_id'] : 2;
        $this->language   = !empty($language['directory']) ? $language['directory'] : 'german';

        // fetch currency
        $currency           = $initHelper->getShopCurrencyFromDatabase();
        $this->exchangeRate = !empty($currency['value']) ? $currency['value'] : 1;
        $this->currencyId   = !empty($currency['currencies_id']) ? $currency['currencies_id'] : 1;
    ...
    private function getCustomFieldsMap($table, array $customFields, $fieldPrefix = '', array $blacklist = array())
    {
        ...
        $qry .= !empty($qrySuffix) ? ' WHERE ' . implode(' AND ', $qrySuffix) : '';
    ...
    public function checkCoupons(ShopgateCart $cart, $customerGroupId)
    {
        ...
        foreach ($cart->getExternalCoupons() as $sgCoupon) {
            ...
            if (empty($coupon)) {
                $sgCoupon->setNotValidMessage(ShopgateLibraryException::getMessageFor(ShopgateLibraryException::COUPON_CODE_NOT_VALID));
        ...
        foreach ($reverse as $cp) {
            if ($cp->getIsValid()) {
                if (!$validCouponFound) {
                    ...
                    $cp->setNotValidMessage(ShopgateLibraryException::COUPON_TOO_MANY_COUPONS);
                    $cp->setIsValid(false);
                }
            }
        }
    ...
    public function checkCartCustomer(ShopgateCart $cart, ShopgateCustomerModel $customerModel)
    {
        ...
        if(empty($customerGroupId)) {
            $customerGroupId = DEFAULT_CUSTOMERS_STATUS_ID;
        }
    ...
    private function getShipping(
        ShopgateCart $shopgateCart, ShopgateItemCartModel $cartItemModel, ShopgateLocationModel $locationModel
    )
    {
        ...
        foreach ($shopgateCart->getItems() as $item) {
            ...
            $completeAmount += $item->getUnitAmount();
                ...
                for ($i = 1; $i <= 100 && isset($internalOrderInfo["attribute_$i"]); $i++) {
                    ...
                    foreach ($option as $optionIds) {
                        $values = $xtPrice->xtcGetOptionPrice(
                            $productId, $optionIds["options_id"], $optionIds["options_values_id"]
                        );
        ...
        if (empty($sgInvoiceAddress)) {
            ...
            $billto = array(
                ...
                "street_address"    => $sgInvoiceAddress->getStreet1(), "suburb" => "",
                "postcode"          => $sgInvoiceAddress->getZipcode(), "city" => $sgInvoiceAddress->getCity(),
                "zone_id"           => $zone["zone_id"],
                "zone_name"         => $zone["zone_name"],
                "country"           => array(
                    "id"         => $country["countries_id"],
    ...
    private function checkCartItems(
        ShopgateCart $cart, ShopgateItemCartModel $cartItemModel, ShopgateLocationModel $locationModel
    ) {
        $resultArr  = array();
        ...
        foreach ($cartItems as $cartItem) {
            ...
            $tax_rate = $locationModel->getTaxRateToProduct(
                $productItem["products_tax_class_id"], false, $this->gambioGXVersion, $this->countryId, $this->zoneId
            );
            ...
            $sgOrderItem->setItemNumber($productItem['products_id']);
            $sgOrderItem->setStockQuantity((int)$productItem["products_quantity"]);
            $sgOrderItem->setQtyBuyable((int)$productItem["products_quantity"]);
            ...
            if (empty($productItem)) {
                ...
                if (STOCK_CHECK == 'false' || STOCK_ALLOW_CHECKOUT == 'true') {
                    ...
                    if ($productItem["products_status"] == 1) {
                        if ((int)$productItem["products_quantity"] < $cartItem->getQuantity()) {
    ...
    protected function cronSetOrdersShippingCompleted(&$message, &$errorCount)
    {
        ...
        while ($shopgateOrder = xtc_db_fetch_array($result)) {
            if (!$this->setOrderShippingCompleted(
                $shopgateOrder['shopgate_order_number'], $shopgateOrder['orders_id'], $this->merchantApi
            )
            ) {
                $errorCount++;
    ...
    protected function setOrderShippingCompleted(
        $shopgateOrderNumber, $orderId, ShopgateMerchantApiInterface &$merchantApi
    ) {
        $success = false;
    ...
    public function addOrder(ShopgateOrder $order)
    {
        ...
        $orderData["shipping_method"] =
            $shippingInfos->getDisplayName() ? $shippingInfos->getDisplayName() : MODULE_PAYMENT_SHOPGATE_TITLE_BLANKET;
        $orderData["shipping_class"]  = $shippingInfos->getName() ? $shippingInfos->getName() : "flat_flat";
        ...
        if ($this->config->getSendOrderConfirmationMail()) {
            ...
            $this->sendOrderConfirmationMail($dbOrderId, $orderData);
        }
     ...
     */
    protected function sendOrderConfirmationMail($orderId, $orderData)
    {
    ...
    private function setOrderPayment(ShopgateOrder $order, $dbOrderId, &$currentOrderStatus)
    {
        ...
        switch ($payment) {
            ...
            case ShopgateOrder::PREPAY:
                ...
                $orderData["payment_method"] = ($paymentWasMapped) ? $paymentName : $paymentMethod;
            ...
            case ShopgateOrder::PAYPAL:
                ...
                $orderData["payment_method"] = ($paymentWasMapped) ? $paymentName : $paypalModuleName;
    ...
    private function updatePaypalOrder(ShopgateOrder $order, $dbOrderId)
    {
        ...
        $paypal_ipn = array(
            'txn_type'        => (!empty($paymentInfo['txn_type']) ? $paymentInfo['txn_type'] : ''),
            'reason_code'     => 'None',
            'payment_type'    => (!empty($paymentInfo['payment_type']) ? $paymentInfo['payment_type'] : ''),
            'payment_status'  => (!empty($paymentInfo['payment_status']) ? $paymentInfo['payment_status'] : ''),
            'pending_reason'  => 'None',
            'invoice'         => (!empty($paymentInfo['invnum']) ? $paymentInfo['invnum'] : null),
            'mc_currency'     => (!empty($paymentInfo['mc_currency']) ? $paymentInfo['mc_currency'] : ''),
            'first_name'      => (!empty($paymentInfo['first_name']) ? $paymentInfo['first_name'] : ''),
            'last_name'       => (!empty($paymentInfo['last_name']) ? $paymentInfo['last_name'] : ''),
            'address_name'    => (!empty($paymentInfo['address_name']) ? $paymentInfo['address_name'] : null),
            'address_street'  => (!empty($paymentInfo['address_street']) ? $paymentInfo['address_street'] : null),
            'address_city'    => (!empty($paymentInfo['address_city']) ? $paymentInfo['address_city'] : null),
            'address_state'   => (!empty($paymentInfo['address_state']) ? $paymentInfo['address_state'] : null),
            'address_zip'     => (!empty($paymentInfo['address_zip']) ? $paymentInfo['address_zip'] : null),
            'address_country' => (!empty($paymentInfo['address_country']) ? $paymentInfo['address_country'] : null),
            'address_status'  => (!empty($paymentInfo['address_status']) ? $paymentInfo['address_status'] : null),
            'payer_email'     => (!empty($paymentInfo['payer_email']) ? $paymentInfo['payer_email'] : ''),
            'payer_id'        => (!empty($paymentInfo['payer_id']) ? $paymentInfo['payer_id'] : ''),
            'payer_status'    => (!empty($paymentInfo['payer_status']) ? $paymentInfo['payer_status'] : ''),
            'payment_date'    => ($order->getPaymentTime() ? $order->getPaymentTime('Y-m-d H:i:s') : ''),
            'business'        => '',
            'receiver_email'  => (!empty($paymentInfo['receiver_email']) ? $paymentInfo['receiver_email'] : ''),
            'receiver_id'     => (!empty($paymentInfo['receiver_id']) ? $paymentInfo['receiver_id'] : ''),
            'txn_id'          => (!empty($paymentInfo['transaction_id']) ? $paymentInfo['transaction_id'] : ''),
            'mc_gross'        => (!empty($paymentInfo['mc_gross']) ? $paymentInfo['mc_gross'] : '0.00'),
            'mc_fee'          => (!empty($paymentInfo['mc_fee']) ? $paymentInfo['mc_fee'] : '0.00'),
            'notify_version'  => '3.7',
            'verify_sign'     => '',
        );
        ...
        $paypal = array(
            ...
            'num_cart_items'      => (!empty($paymentInfo['num_cart_items']) ? $paymentInfo['num_cart_items'] : '0'),
            'settle_currency'     => '',
            'memo'                => '',
            'mc_authorization'    => '0.00',
            'mc_captured'         => '0.00',
            'payment_gross'       => (!empty($paymentInfo['payment_gross']) ? $paymentInfo['payment_gross'] : null),
            'payment_fee'         => (!empty($paymentInfo['payment_fee']) ? $paymentInfo['payment_fee'] : null),
            'exchange_rate'       => '0.00',
        );
        ...
        if ($newTablesExist) {
            ...
            $paypal_ipn_new = array(
                ...
                'date_test'            => '',
                'mc_shipping'          => (!empty($paymentInfo['mc_shipping']) ? $paymentInfo['mc_shipping']
                    : '0.00'),
            );
            ...
            $paypal_transactions = array(
                ...
                'exchange_rate'    => '0.00',
                'raw_response'     => '',
                'grossamount'      => (!empty($paymentInfo['mc_gross']) ? $paymentInfo['mc_gross'] : '0.00'),
                'feeamount'        => (!empty($paymentInfo['mc_fee']) ? $paymentInfo['mc_fee'] : '0.00'),
                'payment_type'     => (!empty($paymentInfo['payment_type']) ? $paymentInfo['payment_type'] : ''),
                'transaction_id'   => (!empty($paymentInfo['transaction_id']) ? $paymentInfo['transaction_id'] : ''),
                'payment_date'     => (!empty($paymentInfo['payment_date']) ? $paymentInfo['payment_date'] : ''),
                'transaction_type' => (!empty($paymentInfo['txn_type']) ? $paymentInfo['txn_type'] : ''),
                'taxamount'        => '0.00 EUR',
            );
            ...
            $orders_paypal = array(
                ...
                'correlation_id' => null,
                'payer_id'       => (!empty($paymentInfo['payer_id']) ? $paymentInfo['payer_id'] : ''),
                'token'          => (!empty($paymentInfo['token']) ? $paymentInfo['token'] : ''),
                'paymentaction'  => '',
            );
            ...
            if ($this->checkColumn('mc_shipping', TABLE_PAYPAL)) {
                $dbData['mc_shipping'] = (!empty($paymentInfo['mc_shipping']) ? $paymentInfo['mc_shipping'] : '0.00');
            }
    ...
    private function insertOrderItems(
        ShopgateOrder $order,
        $dbOrderId, &$currentOrderStatus,
        ShopgateCouponModel $couponModel
    )
    {
        $itemModel = new ShopgateItemModel(
            $this->config, $this->languageId, $this->currency, $this->countryId, $this->zoneId, $this->exchangeRate
        );
        ...
        foreach ($order->getItems() as $orderItem) {
            ...
            $stackQuantity = !empty($order_info['stack_quantity']) ? $order_info['stack_quantity'] : 1;
            $item_number   = isset($order_info["base_item_number"])
                ? $order_info["base_item_number"] : $orderItem->getItemNumber();
            ...
            $productData = array(
                ...
                "products_shipping_time" => xtc_db_prepare_input($dbProduct['shipping_status_name']),
                                ...
                                if (empty($gxCustomizerOrderValuesPerSurface[$customizerSurfaceId][$gxCustomSurfaceValue['elements_values_id']])
                                    ...
                                    $gxCustomizerOrderValuesPerSurface[$customizerSurfaceId][$gxCustomSurfaceValue['elements_values_id']] =
                                        array(
                                            ...
                                            'elements_value'               => '', // leave empty at first
            ...
            if (!empty($options)) {
                ...
                if (!empty($order_info['is_property_attribute'])) {
                    ...
                    $attribute_model = $order_info['products_properties_combis_id'];

                    // Property Daten aus der DB laden
                    $qry
                        = "
                        SELECT
                            ...
                            `pd`.`properties_name`,
                            `pvd`.`values_name`,
                            `pr`.`use_properties_combis_quantity`
                        FROM `" . TABLE_PRODUCTS_PROPERTIES_COMBIS_VALUES . "` AS `ppcv`
                            INNER JOIN `" . TABLE_PRODUCTS_PROPERTIES_COMBIS . "` AS `ppc` ON (`ppcv`.`products_properties_combis_id` = `ppc`.`products_properties_combis_id` AND `ppc`.`products_id` = '$item_number')
                            ...
                            INNER JOIN `" . TABLE_PRODUCTS . "` AS `pr` ON(`pr`.`products_id` = `ppc`.`products_id`)
                        WHERE `ppcv`.`products_properties_combis_id` = '$attribute_model'
                    ...
                    while ($dbProperty = xtc_db_fetch_array($qResult)) {
                        $lastCombiQuantity = $dbProperty['combi_quantity'];
                        // use_properties_combis_quantity == 1 => Products Quantity!
                        switch ($dbProperty['use_properties_combis_quantity']) {
                            case '0': // default stock setting (considers the STOCK_CHECK constant)
                            default:
                                // Do not update items stock based on the STOCK_CHECK setting (no update if check is off)!
                                if (STOCK_CHECK != 'true') {
                                    $noUpdateItemsStock = true;
                                } else {
                                    $noUpdateItemsStock = false;
                                }
                                // Do not update properties stock!
                                $noUpdatePropertiesStock = true;
                                break;
                            case '1': // products stock (ignores the STOCK_CHECK constant)
                                // Do update items stock (because of ignoring the STOCK_CHECK constant in this case)!
                                // Do not update properties stock!
                                $noUpdateItemsStock      = false;
                                $noUpdatePropertiesStock = true;
                                break;
                            case '2': // combi stock
                                // Do not update items stock!
                                // Do update properties stock!
                                $noUpdateItemsStock      = true;
                                $noUpdatePropertiesStock = false;
                                break;
                        ...
                        $productPropertyData = array(
                            'orders_products_id'            => $productsOrderId,
                            'products_properties_combis_id' => $dbProperty['products_properties_combis_id'],
                    ...
                    // Update properties stock (only if set active in stock options)
                    if (STOCK_LIMITED == 'true' && !empty($productPropertyData) && empty($noUpdatePropertiesStock)) {
                        $updateProductsPropertiesCombisData = array(
                            'combi_quantity' => $lastCombiQuantity - $orderItem->getQuantity(),
                        );
                        xtc_db_perform(
                            TABLE_PRODUCTS_PROPERTIES_COMBIS, $updateProductsPropertiesCombisData, 'update',
                            "products_properties_combis_id = '$productPropertyData[products_properties_combis_id]'"
                        );
                    }
        ...
        $this->updateItemsStock($order, $noUpdateItemsStock, $noUpdateAttributes, $noUpdateSpecials);
    ...
    /**
     * @param ShopgateOrder $order
     * @param bool          $ignoreProducts
     * @param bool          $ignoreAttributes
     * @param bool          $ignoreSpecials
     */
    private function updateItemsStock(
        ShopgateOrder $order, $ignoreProducts = false, $ignoreAttributes = false, $ignoreSpecials = false
    ) {
        foreach ($order->getItems() as $item) {
            ...
            if (STOCK_LIMITED == 'true' && !$ignoreProducts) {
                ...
                if ($stock_left <= STOCK_REORDER_LEVEL) {
                    ...
                    xtc_php_mail(
                        STORE_OWNER_EMAIL_ADDRESS, STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, STORE_NAME, '',
                        STORE_OWNER_EMAIL_ADDRESS, STORE_NAME, '', '',
                        $gm_subject, nl2br(htmlentities($gm_body)), $gm_body
                    );
                }
    ...
    private function insertOrderTotal(ShopgateOrder $order, $dbOrderId, ShopgateCouponModel $couponModel)
    {
        ...
        $ordersTotal["title"]      = xtc_db_prepare_input(
            MODULE_PAYMENT_SHOPGATE_ORDER_LINE_TEXT_SHIPPING . ($shippingInfos && $shippingInfos->getDisplayName()
                ...
                : ($shippingInfos && $shippingInfos->getName() ? ' (' . $shippingInfos->getName() . ')' : '')) . ':'
        );
    ...
    /**
     * get the tax rate to the order shipping costs
     *
     * @param ShopgateOrder $order
     *
     * @return float|int
    ...
    private function getOrderShippingTaxRate(ShopgateOrder $order)
    {
        ...
        if (!empty($shippingMethod)) {
            ...
            if (!empty($moduleTaxSetting) && !empty($moduleTaxSetting['configuration_value'])
    ...
    public function checkStock(ShopgateCart $cart)
    {
        ...
        $cartItemModel = new ShopgateItemCartModel($this->languageId);
        $locationModel = new ShopgateLocationModel($this->config);
        $result        = $cartItemModel->getCartItemsFromDatabase($cart);
        $cartItems     = $cart->getItems();

        while ($itemRow = xtc_db_fetch_array($result)) {
            $tax_rate = $locationModel->getTaxrateToProduct(
                $itemRow["products_tax_class_id"], false, $this->gambioGXVersion, $this->countryId, $this->zoneId
            );
            foreach ($cartItems AS $cartItem) {
                $sgOrderItem = new ShopgateCartItem();
                $sgOrderItem->setItemNumber($itemRow["products_id"]);
                $sgOrderItem->setStockQuantity($itemRow["products_quantity"]);

                if ((STOCK_CHECK == "true" && STOCK_ALLOW_CHECKOUT == 'false') && ($itemRow["products_status"] == 1)) {
                    $sgOrderItem->setIsBuyable(true);
                } else {
                    if ($itemRow["products_quantity"] > 0 && $itemRow["products_status"] == 1) {
                        $sgOrderItem->setIsBuyable(true);
                    } else {
                        $sgOrderItem->setError(ShopgateLibraryException::CART_ITEM_OUT_OF_STOCK);
                        $sgOrderItem->setIsBuyable(false);
                ...
                if ($cartItemModel->getProductsIdFromCartItem($cartItem) == $itemRow["products_id"]) {
                    $resultAttributes =
                        $cartItemModel->getCartItemOptions($sgOrderItem, $cartItem->getInternalOrderInfo(), $tax_rate);
                    if (count($resultAttributes) > 0) {
                        $sgOrderItem->setOptions($resultAttributes);
                    }

                    $fields = $cartItemModel->getCartInputFields($itemRow["products_id"], $this->languageId, $tax_rate);
                    if (count($fields) > 0) {
                        $sgOrderItem->setInputs($fields);
                    }
                }
                $resultArr[] = $sgOrderItem;
            }
        }
    ...
    protected function createItemsCsv()
    {
        ...
        while ($item = xtc_db_fetch_array($query)) {
            ...
            $tax_rate = $locationModel->getTaxRateToProduct(
                $item["products_tax_class_id"], false, $this->gambioGXVersion, $this->countryId, $this->zoneId
            );
            ...
            $deepLink = $itemModel->generateDeepLink(
                $item['products_id'], $item['products_name'], $this->gmSEOBoostProductsEnabled, $this->gmSEOBoost
            );
...
//                                }
                        ...
                        $itemArr["item_number"] = $parentItem["item_number"] . ($isFirst ? "" : "_" . $hash);
                        ...
                        $itemArr['properties'] = $itemModel->buildProperties($item);
    ...
    {
        $stackQuantity = (!empty($stackQuantity) ? ceil($stackQuantity) : 1);
        ...
        if ($stackQuantity > 1) {
            ...
            $product['item_name'] .= sprintf(
                " (" . SHOPGATE_PLUGIN_ITEM_NAME_ADDITION_STACK_QUANTITY_INFO . ")", $stackQuantity
            );
