        ...
        for ($i=0; $i<$size; $i+=2) {
          if ($shipping_weight <= $zones_table[$i]) {
            $shipping = $zones_table[$i+1];
			  $shipping_method = MODULE_SHIPPING_ZONES_TEXT_WAY . ' ' . $dest_country . ' : ('.$shipping_num_boxes .' x '. $shipping_weight . ' ' . MODULE_SHIPPING_ZONES_TEXT_UNITS.')';
            break;
          }
        }
