...
$languageTextManager->init_from_lang_file('hpbs', $_SESSION['languages_id']);
...
class hpbs_ORIGIN{
	...
	// class constructor
	function hpbs_ORIGIN(){
		GLOBAL $order, $language;
		
		$this->hgw 				= new heidelpayGW();
		$this->pm 					= 'bs';
		$this->code 				= 'hp'.$this->pm;
		$this->title_txt 			= MODULE_PAYMENT_HPBS_TEXT_TITLE;
		$this->title 				= sprintf(HGW_LOGO, DIR_WS_CATALOG) . $this->title_txt;
		$this->description		= MODULE_PAYMENT_HPBS_TEXT_DESC.'<br/><i>['.$this->hgw->version.']</i>'.$this->hgw->modulConfButton;
		...
		$this->prefix				= 'MODULE_PAYMENT_HPBS_';
	...
	function update_status(){
		...
		if(($this->enabled == true) && ((int) MODULE_PAYMENT_HPBS_ZONE > 0)){
			$check_flag = false;
			$sql = xtc_db_query("SELECT zone_id FROM ". TABLE_ZONES_TO_GEO_ZONES ." WHERE geo_zone_id = '". MODULE_PAYMENT_HPBS_ZONE ."' AND zone_country_id = '". $order->billing['country']['id'] ."' ORDER BY zone_id");
	...
	function selection(){
		...
		if($getConf['transactionMode'] == '1' && isset($order) && strpos(strtolower(MODULE_PAYMENT_HPBS_TEST_ACCOUNT), strtolower($order->customer['email_address'])) === false){
			$this->pmAv = false;
	...
	function keys(){
		return array(
			'MODULE_PAYMENT_HPBS_STATUS',
			'MODULE_PAYMENT_HPBS_TEST_ACCOUNT',
			'MODULE_PAYMENT_HPBS_PROCESSED_STATUS_ID',
			'MODULE_PAYMENT_HPBS_PENDING_STATUS_ID',
			'MODULE_PAYMENT_HPBS_CANCELED_STATUS_ID',
			'MODULE_PAYMENT_HPBS_SORT_ORDER',
			'MODULE_PAYMENT_HPBS_ALLOWED',
			'MODULE_PAYMENT_HPBS_ZONE',
		);
	}
}

MainFactory::load_origin_class('hpbs');
?>