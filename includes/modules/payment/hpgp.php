...
class hpgp_ORIGIN{
	...
	function hpgp_ORIGIN(){
		...
		$this->title_txt 			= MODULE_PAYMENT_HPGP_TEXT_TITLE;
		$this->title 				= sprintf(HGW_LOGO, DIR_WS_CATALOG) . $this->title_txt;
		$this->description			= MODULE_PAYMENT_HPGP_TEXT_DESC.'<br/><i>['.$this->hgw->version.']</i>'.$this->hgw->modulConfButton;
		...
		$this->prefix				= 'MODULE_PAYMENT_HPGP_';
	...
	function update_status(){
		...
		if(($this->enabled == true) && ((int) MODULE_PAYMENT_HPGP_ZONE > 0)){
			$check_flag = false;
			$sql = xtc_db_query("SELECT zone_id FROM ". TABLE_ZONES_TO_GEO_ZONES ." WHERE geo_zone_id = '". MODULE_PAYMENT_HPGP_ZONE ."' AND zone_country_id = '". $order->billing['country']['id'] ."' ORDER BY zone_id");
	...
	function selection(){
		...
		// getting template Version to set decide which JS and CSS shold be included
 		...
 		if ($templateVersion === 2.0) {
			...
			if($getConf['transactionMode'] == '1' && isset($order) && strpos(strtolower(MODULE_PAYMENT_HPGP_TEST_ACCOUNT), strtolower($order->customer['email_address'])) === false){
				$this->pmAv = false;
				...
				$content = array(
						...
						<table>
							<tr><td>'.HGW_TXT_ACC_IBAN.':</td><td><input type="text" class="text " value="" id="iban" name="ACCOUNT.IBAN" /></td></tr>
							<tr><td>'.HGW_TXT_ACC_BIC.':</td><td><input type="text" class="text " value="" id="bic" name="ACCOUNT.BIC" /></td></tr>
	...
	function pre_confirmation_check(){
		$_SESSION['HP']['ACCOUNT.IBAN'] = trim($_POST['ACCOUNT_IBAN']);
		$_SESSION['HP']['ACCOUNT.BIC'] = trim($_POST['ACCOUNT_BIC']);
