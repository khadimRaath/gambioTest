...
class hpdc_ORIGIN{
	...
	function hpdc_ORIGIN(){
		...
		$this->title_txt 			= MODULE_PAYMENT_HPDC_TEXT_TITLE;
		$this->title 				= sprintf(HGW_LOGO, DIR_WS_CATALOG) . $this->title_txt;
		$this->description			= MODULE_PAYMENT_HPDC_TEXT_DESC.'<br/><i>['.$this->hgw->version.']</i>'.$this->hgw->modulConfButton;
		...
		$this->prefix				= 'MODULE_PAYMENT_HPDC_';
	...
	function update_status(){
		...
		if(($this->enabled == true) && ((int) MODULE_PAYMENT_HPDC_ZONE > 0)){
			$check_flag = false;
			$sql = xtc_db_query("SELECT zone_id FROM ". TABLE_ZONES_TO_GEO_ZONES ." WHERE geo_zone_id = '". MODULE_PAYMENT_HPDC_ZONE ."' AND zone_country_id = '". $order->billing['country']['id'] ."' ORDER BY zone_id");
	...
	function selection(){
		...
		if($getConf['transactionMode'] == '1' && isset($order) && strpos(strtolower(MODULE_PAYMENT_HPDC_TEST_ACCOUNT), strtolower($order->customer['email_address'])) === false){
			$this->pmAv = false;
...
// 												'<script src="'.GM_HTTP_SERVER . DIR_WS_CATALOG . 'templates/'.$tplPathJs.'/HeidelpayValPayment.js" type="text/javascript"></script>'.
												...
												'<div class="newreg_'.$this->pm.'" '.$hasReg.'>'.
													'<iframe id="paymentFrameIframe_dc" src="'.$getFormUrl['FRONTEND.PAYMENT_FRAME_URL'].'" frameborder="0" scrolling="no">
