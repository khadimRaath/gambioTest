...
$languageTextManager->init_from_lang_file('hppp', $_SESSION['languages_id']);
...
class hppp_ORIGIN {
	...
	function hppp_ORIGIN(){
		...
		$this->title_txt 			= MODULE_PAYMENT_HPPP_TEXT_TITLE;
		$this->title 				= sprintf(HGW_LOGO, DIR_WS_CATALOG) . $this->title_txt;
		$this->description			= MODULE_PAYMENT_HPPP_TEXT_DESC.'<br/><i>['.$this->hgw->version.']</i>'.$this->hgw->modulConfButton;		
		...
		$this->prefix				= 'MODULE_PAYMENT_HPPP_';
	...
	function update_status(){
		...
		if(($this->enabled == true) && ((int) MODULE_PAYMENT_HPPP_ZONE > 0)){
			$check_flag = false;
			$sql = xtc_db_query("SELECT zone_id FROM ". TABLE_ZONES_TO_GEO_ZONES ." WHERE geo_zone_id = '". MODULE_PAYMENT_HPPP_ZONE ."' AND zone_country_id = '". $order->billing['country']['id'] ."' ORDER BY zone_id");
	...
	function selection(){
		...
		if($getConf['transactionMode'] == '1' && isset($order) && strpos(strtolower(MODULE_PAYMENT_HPPP_TEST_ACCOUNT), strtolower($order->customer['email_address'])) === false){
			$this->pmAv = false;
