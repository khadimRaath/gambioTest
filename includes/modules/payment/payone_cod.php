<?php
/* --------------------------------------------------------------
	payone_cod.php 2014-07-15 mabr
	Gambio GmbH
	http://www.gambio.de
...
class payone_cod_ORIGIN extends payone_master {
	...
	function selection() {
		if($this->pg_config['types']['cod']['active'] == 'true') {
			...
			$selection['module'] = $this->pg_config['types']['cod']['name'];
		}
		else {
			$selection = false;
		}
	...
	function confirmation() {
		$confirmation = array(
			'title' => $this->payone->get_text('confirmation_pay_by_cod'),
		);
		return $confirmation;
	}
	...
	function payment_action() {
		...
		$this->payone->log("(pre-)authorizing cod payment");
		...
		$payment_method = new Payone_Api_Request_Parameter_Authorization_PaymentMethod_CashOnDelivery();
		$payment_method->setShippingprovider('DHL'); // hardcoded for now
		...
		if($this->global_config['authorization_method'] == 'auth') {
			...
			$this->payone->log("cod authorize request:\n".print_r($request, true));
			$response = $service->authorize($request);
			$this->payone->log("cod authorize response:\n".print_r($response, true));
		}
		else { // pre-auth
			...
			$this->payone->log("cod preauthorize request:\n".print_r($request, true));
			$response = $service->preauthorize($request);
			$this->payone->log("cod preauthorize response:\n".print_r($response, true));
		}
...
MainFactory::load_origin_class('payone_cod');