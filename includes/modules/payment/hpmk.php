...
$languageTextManager->init_from_lang_file('hpmk', $_SESSION['languages_id']);
...
class hpmk_ORIGIN{
	...
	// class constructor
	function hpmk_ORIGIN(){
		GLOBAL $order, $language;
		
		$this->hgw 					= new heidelpayGW();
		$this->pm 					= 'mk';
		$this->code 				= 'hp'.$this->pm;
		$this->title_txt 		= MODULE_PAYMENT_HPMK_TEXT_TITLE;
		$this->title 				= sprintf(HGW_LOGO, DIR_WS_CATALOG) . $this->title_txt;
		$this->description	= MODULE_PAYMENT_HPMK_TEXT_DESC.'<br/><i>['.$this->hgw->version.']</i>'.$this->hgw->modulConfButton;
		$this->sort_order		= MODULE_PAYMENT_HPMK_SORT_ORDER;
		$this->enabled			= ((MODULE_PAYMENT_HPMK_STATUS == 'True') ? true : false);
		$this->info					= MODULE_PAYMENT_HPMK_TEXT_INFO;
		$this->tmpOrders		= true;
		$this->hgw->actualPaymethod = strtoupper($this->pm);
		$this->prefix				= 'MODULE_PAYMENT_HPMK_';
	...
	function update_status(){
		...
		if(($this->enabled == true) && ((int) MODULE_PAYMENT_HPMK_ZONE > 0)){
			$check_flag = false;
			$sql = xtc_db_query("SELECT zone_id FROM ". TABLE_ZONES_TO_GEO_ZONES ." WHERE geo_zone_id = '". MODULE_PAYMENT_HPMK_ZONE ."' AND zone_country_id = '". $order->billing['country']['id'] ."' ORDER BY zone_id");
	...
	function selection(){
		...
		if($getConf['transactionMode'] == '1' && isset($order) && strpos(strtolower(MODULE_PAYMENT_HPMK_TEST_ACCOUNT), strtolower($order->customer['email_address'])) === false){
			$this->pmAv = false;
			...
			//check if currency is TRY
			if($order->info['currency'] != 'TRY'){
				$this->pmAv = false;
	...
	function get_error(){
		$error = array(
			'title' => MODULE_PAYMENT_HPMK_TEXT_ERROR,
	...
	function keys(){
		return array(
			'MODULE_PAYMENT_HPMK_STATUS',
			'MODULE_PAYMENT_HPMK_TEST_ACCOUNT',
			'MODULE_PAYMENT_HPMK_PROCESSED_STATUS_ID',
			'MODULE_PAYMENT_HPMK_PENDING_STATUS_ID',
			'MODULE_PAYMENT_HPMK_CANCELED_STATUS_ID',
			'MODULE_PAYMENT_HPMK_SORT_ORDER',
			'MODULE_PAYMENT_HPMK_ALLOWED',
			'MODULE_PAYMENT_HPMK_ZONE',
		);
	}
}

MainFactory::load_origin_class('hpmk');
?>