...
class CssConvertNamedColorsMinifierPlugin extends aCssMinifierPlugin
{
	...
	public function apply(aCssToken &$token)
	{
		...
		{
			$token->Value = preg_replace_callback($this->reMatch, array($this, '_transform'), $token->Value);
		}
	...
	/**
	 * Gambio modification for PHP 7 support.
	 *
	 * CssConvertNamedColorsMinifierPlugin::reReplace equivalent for PHP 7 support
	 *
	 * @param $matches
	 *
	 * @return string
	 */
	private function _transform($matches)
	{
		return $matches[1] . $this->transformation[strtolower($matches[2])] . $matches[3];
	}
}
