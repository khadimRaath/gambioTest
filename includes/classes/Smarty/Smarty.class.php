...
class Smarty3 extends Smarty_Internal_TemplateBase
{
	...
	protected function _getGambioUsermodFilePath($p_template)
	{
		...
		if($usermodFilePath != $t_template) return $usermodFilePath;

		if((defined('DIR_FS_CATALOG') && strpos($t_template, DIR_FS_CATALOG) !== 0) || defined('DIR_FS_CATALOG') == false)
		{
			// try relative path
			...
			if($usermodFilePath != $templateDir . $t_template) return $usermodFilePath;
		}
...
require(DIR_FS_CATALOG. 'includes/classes/Smarty/SmartyBC.class.php');
