...
function do_sql($q){
	if($_SESSION['coo_page_token']->is_valid($_REQUEST['page_token']))
	{
...
function display_select($sth,$q){
 ...
 if ($is_sht || $is_shd) {$w='wa';
   ...
   $sqldr.="<div class='dot'>
&nbsp;MySQL Server:
&nbsp;&#183;<a href='$url&page_token=" . $t_page_token . "&q=show+variables'>Show Configuration Variables</a>
&nbsp;&#183;<a href='$url&page_token=" . $t_page_token . "&q=show+status'>Show Statistics</a>
&nbsp;&#183;<a href='$url&page_token=" . $t_page_token . "&q=show+processlist'>Show Processlist</a>";
   ...
   $sqldr.="<br>";
   if ($is_sht) $sqldr.="&nbsp;Database:&nbsp;&#183;<a href='$url&page_token=" . $t_page_token . "&q=show+table+status'>Show Table Status</a>";
   $sqldr.="</div>";
 }
 if ($is_sht){
 ...
 while($row=mysqli_fetch_row($sth)){
   ...
   for($i=0;$i<$fields_num;$i++){
      ...
      if ($is_sht && $v){
         ...
         ."<td><a href=\"$url&page_token=" . $t_page_token . "&q=select+*+from+$vq\">$v</a></td>"
         ."<td>".$row[1]."</td>"
         ."<td align='right'>".$row[4]."</td>"
         ."<td align='right'>".$row[6]."</td>"
         ."<td align='right'>".$row[8]."</td>"
         ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=show+create+table+$vq\">sct</a></td>"
         ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=explain+$vq\">exp</a></td>"
         ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=show+index+from+$vq\">ind</a></td>"
         ."<td>&#183;<a href=\"$url&shex=1&t=$vq\">export</a></td>"
         ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=drop+table+$vq\" onclick='return ays()'>dr</a></td>"
         ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=truncate+table+$vq\" onclick='return ays()'>tr</a></td>"
         ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=optimize+table+$vq\" onclick='return ays()'>opt</a></td>"
         ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=repair+table+$vq\" onclick='return ays()'>rpr</a>";
      }elseif ($is_shd && $i==0 && $v){
         $url='?'.$xurl."&db=$v";
         $v="<a href=\"$url&page_token=" . $t_page_token . "&q=SHOW+TABLE+STATUS\">$v</a></td>"
         ."<td><a href=\"$url&page_token=" . $t_page_token . "&q=show+create+database+`$v`\">scd</a></td>"
         ."<td><a href=\"$url&page_token=" . $t_page_token . "&q=show+table+status\">status</a></td>"
         ."<td><a href=\"$url&page_token=" . $t_page_token . "&q=show+triggers\">trig</a></td>"
         ;
      }else{
       if (is_null($v)) $v="NULL";
...
<?php if ($_SESSION['is_logged'] && $dbh){ ?>
 | <a href="?<?php eo($xurl)?>&page_token=<?php echo $t_page_token; ?>&q=show+databases">Databases</a>: <select name="db" onChange="frefresh()"><option value='*'> - select/refresh -</option><option value=''> - show all -</option>
<?php echo get_db_select($dbn)?></select>
<?php if($dbn){ $z=" &#183; <a href='".hs($self."?$xurl&db=$dbn"); ?>
<?php echo $z.'&q='.urlencode($SHOW_T)?>&page_token=<?php echo $t_page_token; ?>'>show tables</a>
...
<input type="submit" value=" Apply "><input type="button" value=" Cancel " onclick="window.location='<?php eo($self)?>?&page_token=<?php echo $t_page_token; ?>'">
</center>
</div>
</center>
<?php
...
function db_connect($nodie=0){
 ...
 if ($DB['port']) {
    ...
    $port = isset(explode(':', $DB['host'])[1]) && is_numeric(explode(':', $DB['host'])[1]) ? (int)explode(':', $DB['host'])[1] : null;
    $socket = isset(explode(':', $DB['host'])[1]) && !is_numeric(explode(':', $DB['host'])[1]) ? explode(':', $DB['host'])[1] : null;
    $DB['host'] = explode(':', $DB['host'])[0];
    
    $dbh=mysqli_connect($DB['host'],$DB['user'],$DB['pwd'], null, $port, $socket);
 }
 if (!$dbh) {
...
<input type="hidden" name="page_token" value="<?php echo $t_page_token; ?>">
<input type="submit" value=" Download "><input type="button" value=" Cancel " onclick="window.location='<?php eo($self.'?'.$xurl.'&db='.$DB['db'])?>&page_token=<?php echo $t_page_token; ?>'">
</div>
</center>
<?php
 print_footer();
...
<input type="hidden" name="page_token" value="<?php echo $t_page_token; ?>">
<input type="submit" value=" Upload and Import " onclick="return ays()"><input type="button" value=" Cancel " onclick="window.location='<?php eo($self.'?'.$xurl.'&db='.$DB['db'])?>&page_token=<?php echo $t_page_token; ?>'">
</div>
<br><br><br>
<!--
