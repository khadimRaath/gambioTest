<?php
/* --------------------------------------------------------------
   gm_pdf_conditions.php 2015-09-17 gm
   Gambio GmbH
   http://www.gambio.de
	...
	<!-- <table border="0" width="100%" cellspacing="0" cellpadding="2" class="normalize-table">
		<tr>
			<td valign="top" align="left" class="main">
				<h2><?php echo MENU_TITLE_CONDITIONS; ?></h2>
			</td>
			...
			<?php foreach($gm_values as $key => $value) {?>
				...
				<tr class="dataTableRow">
					...
					<td valign="top" align="left" class="main dataTableContent" style="width: 50%;">
						<textarea onclick="gm_fadeout_boxes('gm_status')" style="width:600px;" id=<?php echo '"' . $key . '"'; if($key == 'GM_PDF_CONDITIONS' || $key == 'GM_PDF_WITHDRAWAL') { echo 'rows="15"'; } else { echo 'rows="1"'; } ?>><?php echo $value; ?></textarea>
					</td>
				</tr>
			<?php } ?>
		</table>
		<div style="display: block; margin-top: 12px; height: 30px;">
			<input class="btn btn-primary pull-right remove-margin" type="button" value="<?php echo BUTTON_SAVE;?>" onClick="gm_fadeout_boxes('gm_status');gm_post_boxes('<?php echo xtc_href_link('gm_pdf_action.php', 'action=gm_pdf_post&lang_id=' . $lang_id. '&page_token=' . $_SESSION['coo_page_token']->generate_token()); ?>', 'gm_status')">
