...
defined('_VALID_XTC') or die('Direct Access to this location is not allowed.');
  ...
  function checkAttribute($current_value_id, $current_pid, $current_product_option_id) {
    ...
    if ($isFound) {
      while($line = xtc_db_fetch_array($result)) {
        ...
        $dl_sql = xtc_db_query("SELECT products_attributes_maxdays, products_attributes_filename, products_attributes_maxcount FROM ".TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD." WHERE products_attributes_id = '" . $line['products_attributes_id'] . "'") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
  ...
  function rowClass($i) {
    $oddRowClassName = 'attributes-odd';
    $evenRowClassName = 'attributes-even';
	return ($i % 2 == 0) ? $oddRowClassName : $evenRowClassName;
  }
