<?php
/* --------------------------------------------------------------
  image_manipulator_GD2.php 2014-07-17 gm
  Gambio GmbH
  http://www.gambio.de
...
class image_manipulation_ORIGIN
{
	...
	function compile()
	{
		...
		if($this->s !== Null)
		{
			...
			// IF IMAGE TYPE IS PNG
			if($this->k == IMAGETYPE_PNG)
			{
				imagealphablending($this->t, false); 
				imagesavealpha($this->t, true); 
				$transparent = imagecolorallocatealpha($this->t, 255, 255, 255, 127); 
				imagefill($this->t, 0, 0, $transparent); 
			}
	...
	function create()
	{
		if($this->s !== Null)
		{
			if($this->d !== "")
			{
				...
				// CHECK IF IMAGE TYPE IS PNG
				if($this->k == IMAGETYPE_PNG)
				{
					// CREATE PNG
					$result = imagepng($this->t, $this->d); 
				}
				elseif($this->k == IMAGETYPE_GIF){
					// CREATE GIF
					$result = imagegif($this->t, $this->d);
				}
				else
				{
					// CREATE JPEG
...
MainFactory::load_origin_class('image_manipulation');
