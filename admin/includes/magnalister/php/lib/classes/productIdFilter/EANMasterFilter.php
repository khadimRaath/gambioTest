...
class EANMasterFilter extends AbstractProductIdFilter {
	...
	public function getProductIds() {
		if (!defined('MAGNA_FIELD_PRODUCTS_EAN')) {
			// This shop does not support the EAN field. Return an empty array so no produtcs are shown.
			return array();
		}
		...
		return MagnaDB::gi()->fetchArray('
			SELECT DISTINCT p.products_id
			 ...
			 WHERE p.'.MAGNA_FIELD_PRODUCTS_EAN.' IS NOT NULL
			       AND p.'.MAGNA_FIELD_PRODUCTS_EAN.' <> ""
		', true);
